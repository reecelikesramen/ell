...
class InstructorProvider(OpenAIProvider):
    ...
    def translate_from_provider(self,
        ...
        if logger:
            # Don't forget to log for verbose mode!
            logger(instructor_response.model_dump_json())
...
@ell.complex(model="gpt-4o-mini", client=client, response_model=UserDetail)
def extract_user(details : str) -> UserDetail:
    return f"Extract {details}"

ell.init(verbose=True)
message = extract_user("John Doe is 25 years old")
user_details = message.parsed
print(user_details)
